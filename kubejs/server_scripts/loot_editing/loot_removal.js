LootJS.modifiers((event) => {
    event.enableLogging();
    /*const modifiers = event.getGlobalModifiers();
    modifiers.forEach((modifier) => {
        console.log(modifier)
    });*/
    
    //Remove Junk from Abandoned Mineshafts
	event.addLootTableModifier('minecraft:chests/abandoned_mineshaft').removeLoot('farmersdelight:skillet')
	event.addLootTableModifier('minecraft:chests/abandoned_mineshaft').removeLoot('supplementaries:rope')
	event.addLootTableModifier('minecraft:chests/abandoned_mineshaft').removeLoot('farmersdelight:rope')
	event.addLootTableModifier('minecraft:chests/abandoned_mineshaft').removeLoot('farmersdelight:cooking_pot')
	event.addLootTableModifier('minecraft:chests/abandoned_mineshaft').removeLoot('vegandelight:tofu')

    //Remove Junk from Bastion Hoglin Stables
    event.addLootTableModifier('minecraft:chests/bastion_hoglin_stable').removeLoot('vegandelight:cooked_tofu')
    event.addLootTableModifier('minecraft:chests/bastion_hoglin_stable').removeLoot('vegandelight:smoked_tofu')

    //Remove Junk from Woodland Mansions
    event.addLootTableModifier('minecraft:chests/woodland_mansion').removeLoot('minecraft:beetroot_seeds')

    //Remove all Chalk from Chests
    event.removeGlobalModifier('@chalk')

    //Remove all Plushies from Chests
    event.removeGlobalModifier('@perfectplushies')

    //Remove all Photographs from Chests
    event.removeGlobalModifier('@exposure')

    
    event.addLootTypeModifier(LootType.CHEST).removeLoot('capsule:capsule')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('tombstone:bag_of_seeds')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('create:dough')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:bacon')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:beetroot_bag')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:carrot_bag')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:chicken_parts')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:dough')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:fertilized_farmland')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:fertilized_soil')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:lamb_ham')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:minced_beef')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:nettle_tea_cup')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:onion')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:onion_bag')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:plow')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:potato_bag')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:raw_pasta')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:ribwort_tea_cup')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:rotten_tomato')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:strawberry_tea_cup')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:supply_cart')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:tomato')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:tomato_bag')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:tomato_seeds')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:wild_onions')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('farm_and_charm:wild_tomatoes')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('forbidden_arcanus:bat_wing')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('forbidden_arcanus:quantum_catcher')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('galosphere:spectre_flare')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('nameless_trinkets:electric_paddle')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('nameless_trinkets:explosion_proof_jacket')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('nameless_trinkets:four_leaf_clover')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('nameless_trinkets:ghast_eye')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('nameless_trinkets:gods_crown')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('nameless_trinkets:missing_page')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('nameless_trinkets:mysterious_trinket')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('nameless_trinkets:sleeping_pills')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('nameless_trinkets:what_magnet')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('reliquary:midas_touchstone')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('reliquary:pyromancer_staff')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('reliquary:rending_gale')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('reliquary:rod_of_lyssa')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('reliquary:zombie_heart')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('supplementaries:rope')
    event.addLootTypeModifier(LootType.CHEST).removeLoot('supplementaries:rope')

   /*
    let toRemove = [
        'abyssal_decor:amaranth_1',
      'abyssal_decor:amaranth_crate',
      'abyssal_decor:amaranth_pinnacle',
      'abyssal_decor:fever_blossom',
      'abyssal_decor:fever_blossom_crop_1',
      'abyssal_decor:fever_blossom_tea',
      'aquamirae:maze_rose',
      'aquamirae:poisoned_chakra',
      'candlelight:beetroot_crate',
      'candlelight:carrot_crate',
      'candlelight:potato_crate',
      'cornexpansion:corn_dough',
      'crabbersdelight:lanternfish_barrel',
      'create:dough',
      'dawnoftimebuilder:camellia_leaves',
      'dawnoftimebuilder:camellia_seeds',
      'dawnoftimebuilder:grape',
      'dawnoftimebuilder:grape_seeds',
      'dawnoftimebuilder:maize',
      'dawnoftimebuilder:rice',
      'dawnoftimebuilder:wild_grape',
      'dawnoftimebuilder:wild_maize',
      'delightful:blueberry_sack',
      'delightful:glow_jelly_bottle',
      'delightful:jelly_bottle',
      'easy_villagers:iron_farm',
      'everycomp:mcf/aether_redux/purple_glacia_hedge',
      'everycomp:q/abyssal_decor/hollow_blackwood_log',
      'everycomp:q/abyssal_decor/hollow_white_wood_log',
      'extra_compat:reinforced_gold_knife',
      'extra_compat:reinforced_gold_machete',
      'farmingforblockheads:feeding_trough',
      'farmingforblockheads:fertilized_farmland_healthy',
      'farmingforblockheads:fertilized_farmland_healthy_stable',
      'farmingforblockheads:fertilized_farmland_rich',
      'farmingforblockheads:fertilized_farmland_rich_stable',
      'farmingforblockheads:fertilized_farmland_stable',
      'farmingforblockheads:green_fertilizer',
      'farmingforblockheads:red_fertilizer',
      'farmingforblockheads:yellow_fertilizer',
      'farm_and_charm:bacon',
      'farm_and_charm:beetroot_bag',
      'farm_and_charm:carrot_bag',
      'farm_and_charm:chicken_parts',
      'farm_and_charm:dough',
      'farm_and_charm:fertilized_farmland',
      'farm_and_charm:fertilized_soil',
      'farm_and_charm:lamb_ham',
      'farm_and_charm:minced_beef',
      'farm_and_charm:nettle_tea_cup',
      'farm_and_charm:onion',
      'farm_and_charm:onion_bag',
      'farm_and_charm:plow',
      'farm_and_charm:potato_bag',
      'farm_and_charm:raw_pasta',
      'farm_and_charm:ribwort_tea_cup',
      'farm_and_charm:rotten_tomato',
      'farm_and_charm:strawberry_tea_cup',
      'farm_and_charm:supply_cart',
      'farm_and_charm:tomato',
      'farm_and_charm:tomato_bag',
      'farm_and_charm:tomato_seeds',
      'farm_and_charm:wild_onions',
      'farm_and_charm:wild_tomatoes',
      'forbidden_arcanus:bat_wing',
      'forbidden_arcanus:quantum_catcher',
      'framedblocks:powered_framing_saw',
      'functionalstorage:armory_cabinet',
      'galosphere:raw_silver',
      'galosphere:raw_silver_block',
      'galosphere:silver_block',
      'galosphere:silver_ingot',
      'galosphere:silver_nugget',
      'galosphere:spectre_flare',
      'herbalbrews:jug',
      'herbalbrews:mixed_tea_leaf_block',
      'itemfilters:always_false',
      'itemfilters:always_true',
      'itemfilters:and',
      'itemfilters:block',
      'itemfilters:custom',
      'itemfilters:damage',
      'itemfilters:id_regex',
      'itemfilters:max_count',
      'itemfilters:mod',
      'itemfilters:not',
      'itemfilters:or',
      'itemfilters:strong_nbt',
      'itemfilters:tag',
      'itemfilters:weak_nbt',
      'itemfilters:xor',
      'justoutdoorstuffs:garden_bag_fertilizers',
      'justoutdoorstuffs:garden_hose',
      'justoutdoorstuffs:garden_lawn_mower',
      'justoutdoorstuffs:patio_bbq_grill',
      'justoutdoorstuffs:patio_modern_planter_black',
      'justoutdoorstuffs:patio_modern_planter_white',
      'meadow:climbing_rope_topmount',
      'minecraft:barrier',
      'minecraft:debug_stick',
      'minecraft:jigsaw',
      'minecraft:light',
      'minecraft:structure_block',
      'minecraft:structure_void',
      'moblassos:contract',
      'moblassos:emerald_lasso',
      'mysticsbiomes:strawberry',
      'mysticsbiomes:strawberry_jam',
      'nameless_trinkets:electric_paddle',
      'nameless_trinkets:explosion_proof_jacket',
      'nameless_trinkets:four_leaf_clover',
      'nameless_trinkets:ghast_eye',
      'nameless_trinkets:gods_crown',
      'nameless_trinkets:missing_page',
      'nameless_trinkets:mysterious_trinket',
      'nameless_trinkets:sleeping_pills',
      'nameless_trinkets:what_magnet',
      'perfectplushies:daniel_plushie',
      'perfectplushies:gamerpotion_plushie',
      'perfectplushies:gamerpotion_plushie_rare',
      'perfectplushies:geode_plushie',
      'perfectplushies:joosh_plushie',
      'perfectplushies:june_plushie',
      'perfectplushies:nyf_plushie',
      'perfectplushies:rocris_plushie',
      'perfectplushies:silk_plushie',
      'perfectplushies:sirjain_plushie',
      'perfectplushies:sirjain_plushie_rare',
      'perfectplushies:sizableshrimp_plushie',
      'perfectplushies:tslat_plushie',
      'pipez:energy_pipe',
      'pipez:gas_pipe',
      'pipez:universal_pipe',
      'regions_unexplored:salmonberry',
      'reliquary:bullets/blaze_bullet',
      'reliquary:bullets/buster_bullet',
      'reliquary:bullets/concussive_bullet',
      'reliquary:bullets/empty_bullet',
      'reliquary:bullets/ender_bullet',
      'reliquary:bullets/exorcism_bullet',
      'reliquary:bullets/neutral_bullet',
      'reliquary:bullets/sand_bullet',
      'reliquary:bullets/seeker_bullet',
      'reliquary:bullets/storm_bullet',
      'reliquary:magazines/blaze_magazine',
      'reliquary:magazines/buster_magazine',
      'reliquary:magazines/concussive_magazine',
      'reliquary:magazines/empty_magazine',
      'reliquary:magazines/ender_magazine',
      'reliquary:magazines/exorcism_magazine',
      'reliquary:magazines/neutral_magazine',
      'reliquary:magazines/sand_magazine',
      'reliquary:magazines/seeker_magazine',
      'reliquary:magazines/storm_magazine',
      'reliquary:midas_touchstone',
      'reliquary:pyromancer_staff',
      'reliquary:rending_gale',
      'reliquary:rod_of_lyssa',
      'reliquary:zombie_heart',
      'sophisticatedbackpacks:advanced_deposit_upgrade',
      'sophisticatedbackpacks:advanced_filter_upgrade',
      'sophisticatedbackpacks:advanced_pump_upgrade',
      'sophisticatedbackpacks:advanced_void_upgrade',
      'sophisticatedbackpacks:auto_smoking_upgrade',
      'sophisticatedbackpacks:deposit_upgrade',
      'sophisticatedbackpacks:filter_upgrade',
      'sophisticatedbackpacks:inception_upgrade',
      'sophisticatedbackpacks:pump_upgrade',
      'sophisticatedbackpacks:smoking_upgrade',
      'sophisticatedbackpacks:stack_upgrade_tier_2',
      'sophisticatedbackpacks:stack_upgrade_tier_3',
      'sophisticatedbackpacks:stack_upgrade_tier_4',
      'sophisticatedbackpacks:void_upgrade',
      'sophisticatedstorage:advanced_feeding_upgrade',
      'sophisticatedstorage:diamond_shulker_box',
      'sophisticatedstorage:feeding_upgrade',
      'sophisticatedstorage:gold_shulker_box',
      'sophisticatedstorage:iron_shulker_box',
      'sophisticatedstorage:netherite_shulker_box',
      'sophisticatedstorage:shulker_box',
      'structure_gel:blue_gel',
      'structure_gel:building_tool',
      'structure_gel:cyan_gel',
      'structure_gel:data_handler',
      'structure_gel:dynamic_spawner',
      'structure_gel:green_gel',
      'structure_gel:orange_gel',
      'structure_gel:red_gel',
      'structure_gel:yellow_gel',
      'supplementaries:rope',
      'tanukidecor:blue_rocket_lamp',
      'tanukidecor:blue_science_pod',
      'tanukidecor:green_rocket_lamp',
      'tanukidecor:green_science_pod',
      'tanukidecor:minimalist_bed',
      'tanukidecor:minimalist_chair',
      'tanukidecor:minimalist_clock',
      'tanukidecor:minimalist_couch',
      'tanukidecor:minimalist_dresser',
      'tanukidecor:minimalist_lamp',
      'tanukidecor:minimalist_mirror',
      'tanukidecor:minimalist_small_table',
      'tanukidecor:minimalist_stool',
      'tanukidecor:minimalist_table',
      'tanukidecor:minimalist_wardrobe',
      'tanukidecor:neon_club_sign',
      'tanukidecor:neon_diamond_sign',
      'tanukidecor:neon_heart_sign',
      'tanukidecor:neon_spade_sign',
      'tanukidecor:orange_science_pod',
      'tanukidecor:orange_science_pod',
      'tanukidecor:pink_rocket_lamp',
      'tanukidecor:purple_rocket_lamp',
      'tanukidecor:red_rocket_lamp',
      'tanukidecor:red_science_pod',
      'tanukidecor:regal_armoire',
      'tanukidecor:turquoise_rocket_lamp',
      'tanukidecor:yellow_rocket_lamp',
      'trashcans:energy_trash_can',
      'unusualprehistory:amber_button',
      'unusualprehistory:ammonite_shell_icon',
      'unusualprehistory:encrusted_flask',
      'unusualprehistory:encrusted_organ',
      'unusualprehistory:encrusted_sack',
      'unusualprehistory:encrusted_spawn_egg',
      'vegandelight:salt',
]

    for (const remove in toRemove) {
        event.addLootTypeModifier(LootType.CHEST).removeLoot(remove)
    }
        */

})
